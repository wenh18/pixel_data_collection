...
class DeviceState(object):
    ...
    def _merge_text(self, view_text, content_description):
        ...
        if view_text:
            view_text = view_text.replace('\n', '  ')
            view_text = f'{view_text[:50]}...' if len(view_text) > 50 else view_text
            text += view_text
            text += ' '
        if content_description:
            content_description = content_description.replace('\n', '  ')
            content_description = f'{content_description[:50]}...' if len(content_description) > 50 else content_description
            text += content_description
        return text
    
    def get_described_actions(self, prefix=None):
        """
        Get a text description of current state
        """
        ...
        for view_id in enabled_view_ids:
            ...
            if editable:
                # view_status += 'editable '
                view_descs.append(input_frame.replace('@', str(len(view_descs))).replace('&', view_class).replace('#', text))
                available_actions.append(SetTextEvent(view=view, text='HelloWorld'))
            elif (clickable or checkable or long_clickable):
                view_descs.append(btn_frame.replace('@', str(len(view_descs))).replace('&', view_class).replace('#', text))
                available_actions.append(TouchEvent(view=view))
            elif scrollable:
    ...
    def get_view_desc(self, view):
        ...
        if editable:
                # view_status += 'editable '
            view_desc = f"<input class='&'>{text}</input>".replace('&', view_class)#.replace('#', text)
            # available_actions.append(SetTextEvent(view=view, text='HelloWorld'))
        elif (clickable or checkable or long_clickable):
            view_desc = f"<button class='&'>{text}</button>".replace('&', view_class)#.replace('#', text)
            # available_actions.append(TouchEvent(view=view))
        elif scrollable:
            view_desc = f"<div class='scroller'>scroll the screen</div>"
        else:
            view_desc = f"<p class='&'>{text}</p>"
        # if content_description:
        #     view_desc += f' "{content_description}"'
        # if view_text:
        #     view_text = view_text.replace('\n', '  ')
    ...
    def get_action_desc(self, action):
        desc = action.event_type
        if isinstance(action, KeyEvent):
            view_desc = "<button id=len(view_descs) class='ImageButton'>go back</button>"
